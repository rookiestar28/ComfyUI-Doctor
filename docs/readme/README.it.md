...
- Le validation errors annidate idonee possono essere mostrate sul subgraph esterno usando dati graph pubblici, mantenendo le informazioni di origine.
- I model asset diagnostics usano in via prioritaria il live model registry di ComfyUI e analizzano subgraph annidati ed estensioni personalizzate con limiti definiti.
- I percorsi dei modelli sono protetti da real-path containment; i valori `Authorization` e `X-API-Key` vengono sempre mascherati.
- Le Doctor settings disattivano la host setting-change telemetry; Desktop runtime identity e private state storage source vengono determinati separatamente.
...
- [Architecture](../ARCHITECTURE.md): host startup, backend/frontend boundaries, data flows, security/storage boundaries e test lanes.
...
- [Validation Guide](../VALIDATION.md): comandi full-gate locali e lanes opzionali focused, integration, stress, compatibility, coverage e audit.
- [Security Audit](../SECURITY_AUDIT.md): quarterly audit cadence, report template, manual checks, optional external scanners e evidence handling.
...
- [Contributing](../../CONTRIBUTING.md): setup, development workflow, validation expectations e PR readiness.
...
Il full gate copre supply-chain checks, secrets detection, pre-commit hooks, host-like startup validation, backend unit tests e frontend Playwright E2E tests. Vedi la [Validation Guide](../VALIDATION.md) per i comandi staged espliciti e le lanes opzionali.

## Requisiti

- Ambiente ComfyUI custom-node.
- Python 3.10 o più recente.
- Node.js `>=18 <26` solo per frontend E2E validation.
