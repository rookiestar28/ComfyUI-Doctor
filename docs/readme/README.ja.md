...
<details>
<summary><strong>新機能: F14 プロアクティブ診断 (ヘルスチェック + インテント署名)</strong></summary>

<details>
<summary><strong>新機能: Data-Driven Diagnostics Signature Packs</strong></summary>

- コードのみではなくデータを通じて診断ツールを維持するための、JSONベースの診断シグネチャパックを追加しました。
- 一般的なComfyUIワークフローの落とし穴（モデルパスの異常、アセットの欠落、アンチパターンなど）に対する制限付きのローカルのみの診断ヒントを追加します。
- 診断結果に、機械可読な信頼度と一致の起源データが含まれるようになりました。

</details>

- **統計 (Statistics)** タブに **診断 (Diagnostics)** セクションを追加し、LLMなしでワークフローの問題をプロアクティブにトラブルシューティングできるようになりました。
- **ヘルスチェック**: ワークフロー検査 (lint)、環境アセット (env assets)、プライバシーチェックを含み、実行可能な修正提案を提供します。
- **インテント署名 (Intent Signature)**: 決定論的なインテント推論システムで、**Top-K インテント + 証拠**を提供し、ワークフローが「何をしようとしているか」を判断します。
- UX強化: 安全なフォールバック（例：「主導的なインテントが検出されません」）および改善された証拠サニタイゼーションを含みます。
...
- 統計 (Statistics) タブに "Trust & Health" パネルを追加
...
診断パネルは、LLM呼び出しを必要としない発見的チェックのための **JSONベースのシグネチャパック** をサポートしています。

- **データ駆動型ルール**: シグネチャパックはバージョン管理されたJSONファイルであり、ルールの更新が容易になります。
- **ローカルのみの拡充**: これらのチェックは診断ヒントのみを追加します（外部への呼び出しやマルウェア判定の主張はありません）。
- **現在の組み込みシグナル**: モデルパスの異常、欠落したアセット、ノード設定のアンチパターン、環境の不一致のヒント。
- **追跡可能な結果**: シグネチャパックの一致には、機械可読な信頼度とメタデータが含まれます。
- **制限されたランタイム**: 無制限のワークフロースキャンを防ぐために、確定的なスキャン上限が適用されます。

#### Quick Community Feedback (GitHub PR)

![Diagnostics](../../assets/feedback.png)

統計タブには、サニタイズされたフィードバックペイロードを準備し、サーバー側からGitHub PRを開くための **Quick Community Feedback** パネルも含まれています。

**機能**:
- 最新のエラー/統計コンテキストから自動的に事前入力します。
- 送信前にサニタイズされたペイロードをプレビューできます。
- 追加専用のフィードバックJSONファイルを送信し、PRを開きます。

**前提条件**:
- サーバー側のGitHubトークン（`DOCTOR_GITHUB_TOKEN`）
- 送信アクション用のAdmin権限
...
- **🛡️ 信頼性と健全性 (Trust & Health)**：`/doctor/health` 指標とプラグイン信頼レポートを表示（スキャンのみ）
- **📊 匿名テレメトリ (Anonymous Telemetry) (建設中 🚧)**：使用状況イベントローカルバッファへのオプトイン（切り替え/表示/クリア/エクスポート）
...
5. 統計タブの一番下までスクロールして、**信頼性と健全性**および**匿名テレメトリ**を見つけます
...
> 注意：**信頼性と健全性 (Trust & Health)** および **匿名テレメトリ (Anonymous Telemetry)** は **統計 (Statistics)** タブに移動しました。

> 注意：**F14 プロアクティブ診断 (Proactive Diagnostics)** は **統計 (Statistics)** タブ → **診断 (Diagnostics)** セクションからアクセスできます。
> **Run / Refresh** を使用してレポートを生成し、問題リストを表示して提供されたアクション（ノードの特定など）を使用します。
> 他の言語でレポートを表示する必要がある場合は、まず設定で **Suggestion Language** を変更してください。
...
**用途:** Advanced Key Store (高度なキーストア) のプロバイダ設定ステータスを取得します。
**認証制限:** 環境に `DOCTOR_ADMIN_TOKEN` が定義されている場合、そのトークンを含める必要があります。
```json
{
    "success": true,
    "providers": {
...
**用途:** キーをローカルサーバーのキーストアに書き込むか、更新します。
**認証制限:** 環境に `DOCTOR_ADMIN_TOKEN` が定義されている場合、そのトークンを含める必要があります。
```json
{
    "provider": "openai",
...
**用途:** ユーザーはフロントエンドで特定の時点のエラーが解決されたかどうかをマークできます。状態には `resolved`, `unresolved`, `ignored` が含まれます。
これにより、バックグラウンドでの一般的な問題の平均解決率の集計に役立ちます。
**Request payload:**
```json
{
...
**用途:** フロントエンドからサニタイズされたクラッシュレポートをエクスポートしてプレビューを作成します。これは、プライベート情報が含まれていないことを確認するための、コミュニティPull Request送信前のチェックフェーズです。
**Request payload:**
```json
{
    "pattern_candidate": {
...
**用途:** 以前に生成されたコミュニティフィードバックレポートを実際にGitHub Pull Requestとして公開し、`ComfyUI-Doctor`コアモジュールに新しいサンプルを提供します。成功させるには、バックエンドで専用の `DOCTOR_GITHUB_TOKEN` を設定する必要があります。
**認証制限:** サーバーの `DOCTOR_ADMIN_TOKEN` 設定と同じ制限。
**Request payload:**
```json
{
    "submission_id": "20260227_...",
...
### GET /doctor/health
**用途:** ノードのヘルスレポートと基本ステータスを取得します (分析は実行しません、スキャンデータのみを提供)。
**Response:**
```json
{
    "success": true,
...
### GET /doctor/plugins
**用途:** 現在有効なアナライザーパック (Analyzer Packs) の信頼リストと認証ステータスを表示します。
**Response:**
```json
{
    "success": true,
...
**用途:** 匿名テレメトリモジュールのスイッチと現在の統計を確認します。
**Response:**
```json
{
  "success": true,
...
**用途:** まだ送信されていない匿名テレメトリキャッシュの内部詳細を確認します。
**Response:**
```json
{
  "success": true,
...
**用途:**フロントエンドから報告されるか、ローカルでエラーがインターセプトされたときに呼び出され、キャッシュに匿名イベントをプッシュします。

### POST /doctor/telemetry/clear (S3)
**用途:** 現在のすべてのローカル匿名テレメトリキャッシュをクリアします。
**Response:**
```json
{ "success": true }
```
...
**用途:** ダウンロードと分析のためにローカルのテレメトリキャッシュを元のJSONファイルとしてエクスポートします。

### POST /doctor/telemetry/toggle (S3)
**用途:** 匿名データテレメトリ収集機能を有効または無効にします。
```json
{
  "enabled": true
}
...
**用途:** 特別なエンドポイント: レポートステータスのAcknowledgeを提供するためのリクエスト
デフォルトのステータスは `acknowledged`, `ignored`, `resolved` です。
