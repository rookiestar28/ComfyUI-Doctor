...
- 통계 (Statistics) 탭에 "Trust & Health" 패널 추가
...
- **🛡️ 신뢰 및 상태 (Trust & Health)**: `/doctor/health` 지표 및 플러그인 신뢰 보고서 확인(스캔 전용)
- **📊 익명 원격 측정 (Anonymous Telemetry) (공사 중 🚧)**: 사용 이벤트 로컬 버퍼에 대한 선택적 참여(토글/보기/지우기/내보내기)
...
5. 통계 탭 하단으로 스크롤하여 **신뢰 및 상태**와 **익명 원격 측정** 섹션을 찾습니다.
