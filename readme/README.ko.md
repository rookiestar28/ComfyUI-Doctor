...
### 10. 신뢰 및 상태 (Trust & Health)

**기능**: 시스템 상태 및 플러그인 신뢰 보고서를 봅니다.
**사용법**: 🔄 새로 고침 버튼을 클릭하여 `/doctor/health` 데이터를 가져옵니다.

**표시 내용**:

- **Pipeline Status**: 현재 분석 파이프라인 상태
- **SSRF Blocked**: 차단된 의심스러운 아웃바운드 요청 수
- **Dropped Logs**: 배압(backpressure)으로 인해 삭제된 로그 메시지 수
- **Plugin Trust List**: 신뢰 상태 배지와 함께 감지된 모든 플러그인 표시:
  - 🟢 **Trusted**: 유효한 매니페스트가 있는 허용 목록 플러그인
  - 🟡 **Unsigned**: 매니페스트가 없는 플러그인 (주의해서 사용)
  - 🔴 **Blocked**: 차단 목록에 있는 플러그인

### 11. 익명 원격 측정 (공사 중 🚧)

**기능**: Doctor 개선을 돕기 위해 익명 사용 데이터 수집에 참여합니다.
**상태**: **공사 중** — 현재 로컬 전용이며 네트워크 업로드 없음.

**제어**:

- **Toggle**: 원격 측정 기록 활성화/비활성화 (기본값: OFF)
- **View Buffer**: 업로드 전 버퍼링된 이벤트 검사
- **Clear All**: 버퍼링된 모든 원격 측정 데이터 삭제
- **Export**: 검토를 위해 버퍼링된 데이터를 JSON으로 다운로드

**개인정보 보호 보장**:

- ✅ **수신 동의(Opt-in) 전용**: 명시적으로 활성화할 때까지 데이터가 기록되지 않음
- ✅ **로컬 전용**: 현재 로컬에만 데이터 저장 (`Upload destination: None`)
- ✅ **PII 감지**: 민감한 정보를 자동으로 필터링
- ✅ **완전한 투명성**: 향후 업로드 전에 모든 데이터 보기/내보내기.
