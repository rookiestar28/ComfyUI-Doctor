...
### 10. 信任與健康狀態 (Trust & Health)

**功能**：查看系統健康狀態和插件信任報告。
**用法**：點擊 🔄 重新整理按鈕以獲取 `/doctor/health` 端點數據。

**顯示內容**：

- **Pipeline Status**：目前的分析管線狀態
- **SSRF Blocked**：被攔截的可疑外連請求計數
- **Dropped Logs**：因背壓 (backpressure) 而丟棄的日誌訊息計數
- **Plugin Trust List**：顯示所有檢測到的插件及其信任狀態標記：
  - 🟢 **Trusted**：已列入白名單且具備有效清單的插件
  - 🟡 **Unsigned**：無清單的插件（請謹慎使用）
  - 🔴 **Blocked**：列入黑名單的插件

### 11. 匿名遙測 (建設中 🚧)

**功能**：選擇性加入匿名使用數據收集，以協助改進 Doctor。
**狀態**：**建設中** — 目前僅本地存儲，無網絡上傳。

**控制項**：

- **Toggle**：啟用/禁用遙測記錄（預設：關閉）
- **View Buffer**：在上傳前檢查緩衝的事件
- **Clear All**：刪除所有緩衝的遙測數據
- **Export**：下載緩衝數據為 JSON 以供審查

**隱私保證**：

- ✅ **僅限加入**：除非顯式啟用，否則不會記錄任何數據
- ✅ **僅限本地**：目前僅將數據存儲在本地 (`Upload destination: None`)
- ✅ **PII 檢測**：自動過濾敏感信息
- ✅ **完全透明**：在任何未來上傳之前查看/導出所有數據
