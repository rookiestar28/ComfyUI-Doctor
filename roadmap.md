...
- [x] **S10**: Write-Sensitive Endpoint Admin Guard Consolidation - High *Completed (2026-02-27; TEST_SOP full gate passed)*
  - **Outcome**: All listed write-sensitive state mutation routes now enforce unified admin guard policy before mutation.
  - **Delivered**:
    - Guarded routes in `__init__.py`:
      - `/doctor/statistics/reset`
      - `/doctor/mark_resolved`
      - `/doctor/telemetry/clear`
      - `/doctor/telemetry/toggle`
      - `/doctor/health_ack`
    - Guarded routes in `services/routes.py`:
      - `/doctor/jobs/{job_id}/resume`
      - `/doctor/jobs/{job_id}/cancel`
    - Standardized denial semantics via helper (`401` unauthorized token path, `403` remote denied path).
    - Added regression tests:
      - Runtime auth behavior tests for resume/cancel (`tests/test_t20_api.py`)
      - Static guard coverage checks for all S10 routes (`tests/test_s10_admin_guards.py`)
  - **Plan**: `.planning/260227-S10_WRITE_SENSITIVE_ADMIN_GUARD_PLAN.md`
  - **Implementation Record**: `.planning/260227-S10_WRITE_SENSITIVE_ADMIN_GUARD_IMPLEMENTATION_RECORD.md`
- [x] **S11**: SSRF DNS-Rebinding Hardening for Outbound URL Validation - High *Completed (2026-02-27; TEST_SOP full gate passed)*
  - **Outcome**: SSRF validation now applies hostname DNS resolution with fail-closed policy and post-resolution restricted-IP blocking.
  - **Delivered**:
    - `security.py`:
      - Added bounded DNS resolution helper (`_resolve_hostname_ips`) with timeout control.
      - Added restricted IP classifier for private/loopback/link-local/multicast/reserved/metadata endpoint ranges.
      - Extended `validate_ssrf_url()` to reject hostname targets resolving to restricted IPs.
      - Added fail-closed behavior for DNS timeout/failure (`Blocked: DNS resolution error ...`).
      - Added env knob: `DOCTOR_SSRF_DNS_TIMEOUT_SECONDS` (bounded).
    - Tests:
      - `tests/test_api_endpoints.py` now includes deterministic DNS-rebinding regression cases and DNS-timeout fail-closed check.
      - Cloud URL allow-case made offline-safe via mocked DNS resolution.
  - **Plan**: `.planning/260227-S11_SSRF_DNS_REBINDING_HARDENING_PLAN.md`
  - **Implementation Record**: `.planning/260227-S11_SSRF_DNS_REBINDING_HARDENING_IMPLEMENTATION_RECORD.md`
- [x] **S8**: Dual-Mode API Key Strategy (ENV-first + Server Store Fallback) - ???High ??*Completed (2026-02-10)*
  - **Outcome**: API keys are no longer persisted in frontend settings; frontend key input is session-only by default.
  - **Delivered**:
    - Backend modules: `services/secret_store.py`, `services/llm_keys.py`, `services/admin_guard.py`
    - Secrets endpoints: `GET /doctor/secrets/status`, `PUT /doctor/secrets`, `DELETE /doctor/secrets/{provider}`
    - Key precedence: request key ??provider-specific ENV ??generic ENV ??server store
    - Frontend: default-collapsed **Advanced Key Store (Server-side)** section with explicit risk warning and admin-token support
    - Legacy migration: one-time read+clear of `Doctor.LLM.ApiKey` to runtime memory with migration notice
  - **Plan**: `.planning/260210-S8_DUAL_MODE_API_KEY_STRATEGY_PLAN.md`
  - **Implementation Record**: `.planning/260210-S8_DUAL_MODE_API_KEY_STRATEGY_IMPLEMENTATION_RECORD.md`
  - **References**:
    - `docs/reference/260205-S25_DUAL_MODE_SECRETS_RECORD.md`
    - `docs/reference/260205-S26_SIMPLIFIED_SETTINGS_RECORD.md`
- [x] **S9**: External Enrichment Submission Safety Policy (Opt-in by Action) - High *Completed (2026-02-15 implementation; TEST_SOP full gate passed 2026-02-16)*
  - **Goal**: Keep Doctor safe-by-default when integrating optional external security/intel providers.
  - **Plan**: `.planning/260215-S9A10R20T15_EXTERNAL_ENRICHMENT_SAFETY_FOUNDATION_PLAN.md`
  - **Implementation Record**: `.planning/260215-S9A10R20T15_EXTERNAL_ENRICHMENT_SAFETY_FOUNDATION_IMPLEMENTATION_RECORD.md`
  - **Policy**:
    - Query/read actions allowed by default; submit/upload actions default OFF.
    - Submit/upload requires explicit user action + per-provider allow switch.
    - Enforce endpoint allowlist + strict SSRF boundary for all provider calls.
    - Redacted audit trail for provider actions (who/when/provider/action, never raw secrets/content).
  - **ComfyUI-Doctor Adaptation (merged high-value scope)**:
    - Add provider capability policy (`read/query/fetch/status` vs `submit/upload`) as server-side source of truth; UI must not be the trust boundary.
    - Route all provider-bound payloads through `outbound.py` sanitization + local/remote privacy guardrails; submit paths cannot bypass this funnel.
    - Require explicit confirmation token for each submit/upload action (short TTL, single-use), plus admin guard where configured.
    - Persist a redacted action audit log in Doctor data dir (timestamp, provider, action, policy decision, payload digest only).
    - Keep default UX read-only: no implicit submit side effects during analyze/chat/health flows.
  - **Integration Note**:
    - Merged with **A10** adapter design and **T5/T15** test gates; no separate duplicate item created.
  - **Reference Rationale**:
    - `docs/reference/malwoverview/README.md` (explicit non-submit default stance)
    - Added reference batch timestamp: `docs/reference/*` on 2026-02-14
    - License note: reference behavior only; implementation remains native to Doctor codebase.
- [x] **S6**: PII Sanitization for LLM traceback - ???High ??*Completed (2025-12-31)*
  ...
  - **Frontend** (Privacy Controls):
    ...
    - ??? "Preview" button: Not implemented (deferred to future)
    - ??? Audit log UI: Not implemented (backend logging only)
...
- [x] **S12**: Server Secret Store Hardening (ACL + Optional At-Rest Encryption) - Medium *Completed (2026-02-27; TEST_SOP full gate passed)*
  - **Outcome**: Server-side secret storage now supports optional encryption-at-rest with compatibility-preserving migration and platform permission hardening guardrails.
  - **Delivered**:
    - `services/secret_store.py`:
      - Added optional encrypted payload envelope for `secrets.json` with integrity verification.
      - Added fail-closed write policy when `DOCTOR_SECRET_STORE_ENCRYPTION_REQUIRED=1` but encryption key is missing.
      - Added runtime hardening diagnostics (plaintext warning + missing-key error path).
      - Added Windows ACL best-effort hardening path (`icacls`) with per-instance single-attempt behavior.
    - Regression tests in `tests/test_s8_secrets.py`:
      - encryption roundtrip (no plaintext leakage)
      - required-encryption rejection without key
      - plaintext legacy load + encrypted migration
      - Windows ACL hardening call-path coverage
  - **Plan**: `.planning/260227-S12_SECRET_STORE_HARDENING_PLAN.md`
  - **Implementation Record**: `.planning/260227-S12_SECRET_STORE_HARDENING_IMPLEMENTATION_RECORD.md`
- [x] **S13**: Outbound Proxy Trust Boundary Hardening (`trust_env`) - Low *Completed (2026-02-27; TEST_SOP full gate passed)*
  - **Outcome**: Shared outbound session no longer inherits environment proxy settings by default; proxy inheritance is now explicit and observable.
  - **Delivered**:
    - `session_manager.py`:
      - Added explicit proxy policy contract: `strict_off` (default) and `inherit_env` (opt-in).
      - Added policy precedence: env override (`DOCTOR_OUTBOUND_PROXY_POLICY`) > config (`llm_proxy_policy`) > secure fallback.
      - Bound `aiohttp.ClientSession(..., trust_env=...)` to effective policy.
      - Added diagnostics helper to expose effective policy/source.
    - `config.py`:
      - Added `llm_proxy_policy` with secure default `strict_off`.
    - `__init__.py`:
      - Wired startup policy configuration.
      - Extended `/doctor/health` with `outbound_proxy` diagnostics payload.
    - Tests:
      - Expanded `tests/test_session_manager.py` with default/override/fallback and trust_env wiring coverage.
  - **Plan**: `.planning/260227-S13_OUTBOUND_PROXY_TRUST_BOUNDARY_HARDENING_PLAN.md`
  - **Implementation Record**: `.planning/260227-S13_OUTBOUND_PROXY_TRUST_BOUNDARY_HARDENING_IMPLEMENTATION_RECORD.md`
...
- [x] **S1**: Document CSP Compliance/Limitations - ???Low ??*Code Audit Complete (2026-01-09)*
  ...
  - ??? **Pending**: Manual verification with `--disable-api-nodes` + screenshots (user required)
...
- [x] **R19**: LLM Output Contract Hardening (Truncation Cleanup + Structured Fallback) - High *Completed (2026-02-15 implementation; TEST_SOP full gate passed 2026-02-16)*
  - **Goal**: Prevent partial/dirty LLM outputs from leaking to UI and breaking downstream parsing.
  - **Scope**:
    - Strip hidden reasoning/thinking blocks and incomplete marker fragments before render/store (SSE + non-stream responses).
    - Add deterministic response contract validator (minimum sections + typed structure) before UI render and persistence.
    - Detect truncated/invalid structured output and retry once with bounded fallback strategy (reduced context and/or adjusted generation budget).
    - Add deterministic fallback output (`summary/actions/evidence`) when contract validation fails after retry.
    - Add bounded extraction caps for evidence blocks (lines/chars/items) to reduce noisy payloads.
  - **ComfyUI-Doctor Adaptation (merged high-value scope)**:
    - Must tolerate mixed provider behavior (OpenAI-compatible, Anthropic style, local LLM quirks) without breaking sidebar rendering.
    - Must keep first-token latency stable for chat UX; cleanup/validation cost stays bounded.
    - Must preserve privacy guarantees: no hidden reasoning blocks or raw internal traces are re-exposed after cleanup.
  - **References**:
    - `docs/reference/Qu1cksc0pe/README.md` (AI output cleanup + compacting controls)
    - `docs/reference/Qu1cksc0pe/Modules/analysis/multiple/smart_analyzer.py`
    - License note: reference logic only; no GPL code copy.
  - **Implementation Note (2026-02-15)**:
    - Hidden-marker cleanup is enforced on `/doctor/analyze` and non-stream `/doctor/chat`; SSE chunk-level filtering remains deferred to follow-up.
- [x] **R20**: Interrupt-Safe Checkpoint/Resume for Long-Running Enrichment Jobs - High *Completed (2026-02-15 implementation; TEST_SOP full gate passed 2026-02-16)*
  - **Goal**: Allow long provider pulls/enrichment runs to resume safely after interruption.
  - **Plan**: `.planning/260215-S9A10R20T15_EXTERNAL_ENRICHMENT_SAFETY_FOUNDATION_PLAN.md`
  - **Implementation Record**: `.planning/260215-S9A10R20T15_EXTERNAL_ENRICHMENT_SAFETY_FOUNDATION_IMPLEMENTATION_RECORD.md`
  - **Scope**:
    - Persist checkpoint cursor + pending queue in Doctor data dir with atomic updates.
    - Resume from last confirmed cursor; avoid replay via deterministic dedup key/index.
    - Add explicit cancel/resume controls and stale-checkpoint cleanup policy.
  - **ComfyUI-Doctor Adaptation (merged high-value scope)**:
    - Use crash-safe state files (versioned JSON + tmp/rename) under resolved Doctor data dir (Desktop/portable/git compatible).
    - Ensure safe interruption semantics for ComfyUI restart, extension reload, and Ctrl+C/server stop.
    - Add bounded checkpoint GC and corruption recovery (rotate + rebuild) aligned with existing store guardrails.
    - Keep worker loop non-blocking for UI thread; resume should not duplicate already acknowledged provider items.
  - **Reference**:
    - `docs/reference/waymore/waymore/waymore.py` (`continueResp*.tmp` + resume flow)
- [x] **R17 (P1)**: Limited Config Externalization (Compatibility Guardrails) - Medium *Completed (2026-02-17; TEST_SOP gate passed)*
  - **Sprint Plan**: `.planning/260216-R17T13T9T5_NEXT_SPRINT_VALIDATION_EXPANSION_PLAN.md`
  - **Implementation Record**: `.planning/260217-R17T13T9T5_REMEDIATION_RECORD.md` *(remediation update; supersedes earlier partial record for current status)*
  - **Status (2026-02-17)**: Implemented with runtime guardrail wiring across job retention, aggregation windows, provider timeout/retry defaults, and rate-limit window configuration; TEST_SOP gate passed.
  ...
  - **Scope**:
    - Consolidate runtime guardrails into a single config surface (prefer request-local settings; avoid global side-effects):
      ...
      - Path/persistence policy knobs used by **R16** and **R20** (resolved data dir, migration toggles, checkpoint retention)
    - Support safe overrides via **ComfyUI settings** and/or env vars where appropriate (defaults unchanged).
    - Add provider-profiled network guardrails (per-provider timeout/retry/backoff/rate-limit caps) so remote/local/API-vendor behavior can be tuned without code edits.
...
- [ ] **R21**: UTC-Aware Datetime Standardization (`datetime.utcnow()` Migration) - Low
  ...
  - **Scope**:
    ...
    - Normalize comparison logic between aware/naive timestamps in history/statistics/telemetry paths.
    - Add regression tests for timestamp ordering and retention windows across mixed legacy records.
  - **Acceptance**:
...
- [x] **R22**: Logger Asyncio Transport GC Recursive Pollution Fix - ⚡High *Completed (2026-03-25)*
  ...
  - **Scope (2 phases)**:
    - **R22-P1**: Add exclusion filter in `DoctorLogProcessor._process_message()` for `_ProactorBasePipeTransport.__del__` + `I/O operation on closed pipe`. Align with existing Desktop flush-ignore filter.
    - **R22-P2**: Refactor `_record_analysis` to use a dedicated `logging.Logger` instance without `StreamHandler` (or write directly to log file), preventing stream-level interleaving. Add thread-local reentrance guard in `SafeStreamWrapper.write()`.
  - **Acceptance**:
    - asyncio transport GC warnings no longer trigger error analysis loops.
    - `_record_analysis` output does not appear interleaved within captured exception tracebacks.
    - Existing error capture functionality remains unaffected (regression tests pass).
...
- [ ] **R24**: Desktop Virtual Environment Path Compatibility Verification - Low
  ...
  - **Scope**:
    ...
    - Confirm `system_info.py` correctly detects new Desktop installation modes.
    - Add test fixtures for new Desktop path scenarios if gaps are found.
  - **Acceptance**:
...
- [x] **F16**: Quick Community Feedback (GitHub PR) - ???High *Completed (2026-02-26; TEST_SOP full gate passed)*
  ...
  - **Original Draft Plan**: `.planning/260108-F16_GITHUB_FEEDBACK_PR_PLAN.md`
  - **Plan**: `.planning/260226-F16_GITHUB_PR_FEEDBACK_PLAN.md`
  - **Implementation Record**: `.planning/260226-F16_GITHUB_PR_FEEDBACK_IMPLEMENTATION_RECORD.md`
...
- [x] **F18**: Data-Driven Signature Packs for Heuristic Diagnostics - High *Completed (2026-02-27; TEST_SOP full gate passed)*
  - **Goal**: Expand proactive diagnostics with maintainable JSON signature packs (no code release needed for every rule tweak).
  - **Scope**:
    - Add versioned JSON signature packs for suspicious artifact/noise heuristics with category metadata.
    - Support pack enable/disable + confidence levels + provenance tags in diagnostics output.
    - Keep this as diagnostic enrichment only (no malware verdict claim).
    - Reuse existing pattern validation workflow for schema/lint checks.
    - Add ComfyUI-aware signal families first (model path anomalies, missing assets, node config anti-patterns, environment mismatch hints).
    - Keep runtime bounded: deterministic caps per check, no unbounded scan over full workflow/log corpus.
  - **Integration Note**:
    - Merged with **F14** diagnostics pipeline and **T8/T15** validation strategy; no duplicate parallel feature item.
  - **References**:
    - `docs/reference/Qu1cksc0pe/Systems/Multiple/special_artifact_patterns.json`
    - `docs/reference/Qu1cksc0pe/Systems/Windows/windows_api_categories.json`
  - **Plan**: `.planning/260226-F18_SIGNATURE_PACKS_FOR_DIAGNOSTICS_PLAN.md`
  - **Implementation Record**: `.planning/260226-F18_SIGNATURE_PACKS_FOR_DIAGNOSTICS_IMPLEMENTATION_RECORD.md`
...
- [ ] **F6**: Multi-LLM provider quick switch - ???Medium ??? *Use dev branch*
...
- [x] **A7**: Frontend Architecture Modernization (Preact Migration) - ???Medium ??*Completed (2026-01-08)*
  ...
  - **Migration Strategy**:
    - **Phase 4D**: Planning and Preact loader infrastructure ??    - **Phase 5A**: Migrate Chat & Statistics to Preact islands ??    - **Phase 5B**: Integration hardening with fallbacks ??    - **Phase 5C**: Extensibility layer (registry, error boundaries) ??  - **Technical Approach**:
...
- [x] **A10**: External Provider Adapter Layer (Threat/Intel-style Enrichment) - High *Completed (2026-02-15 implementation; TEST_SOP full gate passed 2026-02-16)*
  - **Goal**: Introduce a clean adapter boundary for optional external enrichment sources without coupling core Doctor flow to provider-specific APIs.
  - **Plan**: `.planning/260215-S9A10R20T15_EXTERNAL_ENRICHMENT_SAFETY_FOUNDATION_PLAN.md`
  - **Implementation Record**: `.planning/260215-S9A10R20T15_EXTERNAL_ENRICHMENT_SAFETY_FOUNDATION_IMPLEMENTATION_RECORD.md`
  - **Scope**:
    - Define provider adapter contract (`search/query/fetch/status`) with normalized response schema.
    - Centralize retries/timeouts/rate-limit/backoff policy at adapter boundary.
    - Separate read/query vs submit/upload capabilities to align with **S9** safety policy.
    - Keep adapters optional and disabled by default unless explicitly configured.
    - Add capability registry per provider (`supports_submit`, `requires_auth`, `max_page_size`, `resume_cursor_type`) as server-side policy.
    - Ensure non-blocking execution model compatible with ComfyUI runtime (cancel/resume hooks for long jobs).
  - **References**:
    - `docs/reference/malwoverview/malwoverview/malwoverview.py` (multi-extractor orchestration)
    - `docs/reference/waymore/waymore/waymore.py` (provider-specific throttling/retry behavior)
    - License note: architecture inspiration only; no GPL code import.
- [ ] **A9**: Frontend Extension API Modernization (ComfyUI_frontend Compatibility) - Medium *Use dev branch*
  ...
  - **Scope**:
    - Migrate settings registration to `ComfyExtension.settings` (avoid deprecated `app.ui.settings.addSetting/getSettingValue/setSettingValue`)
      - **Current**: 13 calls to `app.ui.settings.addSetting()` in `doctor.js` (explicitly `@deprecated` in `ComfyUI_frontend/src/scripts/ui/settings.ts:L98`)
      - **Target**: Declarative `settings: [...]` array in `app.registerExtension()` call
    - Prefer `app.rootGraph` over deprecated `app.graph` for workflow capture and node lookup
      - **Current**: 12 calls to `app.graph` in `doctor_ui.js` + `doctor_fix_handler.js` (explicitly `@deprecated` in `ComfyUI_frontend/src/scripts/app.ts:L212`)
      - **Target**: Replace with `app.rootGraph` (with `app.isGraphReady` guard)
  ...
  - **Acceptance**:
    ...
    - Zero usage of `app.graph` (replaced by `app.rootGraph`)
  ...
  - **Risk Note**: Both deprecated APIs currently have legacy shims — no immediate breakage, but ComfyUI team may remove them in future releases.
  - **Updated**: 2026-03-25 (reference project audit confirmed `@deprecated` annotations)
- [ ] **A5**: Create `LLMProvider` Protocol for unified LLM interface - ???Medium ??? *Use dev branch*
...
- [ ] **A4**: Convert `NodeContext` to `@dataclass(frozen=True)` + validation - ???Medium ??? *Use dev branch*
...
- [x] **T8**: Pattern Validation CI - ???Medium ??*Completed (2026-01-03)*
  ...
  - **Implementation**:
    - JSON schema validation (format correctness) ??    - Regex syntax validation (all patterns compile successfully) ??    - i18n completeness check (all 57 patterns translated in 9 languages) ??    - Pattern metadata validation (priority ranges, valid categories, unique IDs) ??    - GitHub Actions workflow (triggered on push/PR) ??  - **Deliverable**: PR checks fail if validation errors found
...
- [x] **T2**: Frontend interaction tests (Playwright) - ???Medium ??*Completed (2026-01-04)*
  ...
  - **Implementation**:
    - Test harness loads full Doctor UI without ComfyUI ??    - Mock ComfyUI environment (app, api, extensionManager) ??    - Settings panel tests (13 tests): toggle, selectors, persistence, trust/health refresh ??    - Chat interface tests (15 tests): messages, inputs, error context, sanitization status, analyze CTA ??    - Statistics dashboard tests (19 tests): panel, cards, categories, i18n, resolution actions ??    - Preact loader tests (14 tests): module loading, flags, vendor failure fallback ??    - API endpoint mocks for backend calls ??  - **Test Results (latest)**:
    - Required harness suite: `npm test` ??87 passed (`@integration` excluded by default)
    - Optional integration suite: `npm run test:integration` ??backend-dependent telemetry tests
  - **Execution time**: environment-dependent; see `tests/E2E_TESTING_SOP.md` for verified baselines
  - **How to Run Tests**:
    <details>
    ...
    # Run optional backend integration tests
  ...
  - **Follow-up Record**: `.planning/260109-PHASE2_FOLLOWUP_TRUST_HEALTH_UI_AND_E2E_RECORD.md`
  - **SOP**: `tests/E2E_TESTING_SOP.md` (dual-track CI/testing policy)
  - **Foundation for**: CI/CD integration, UI regression detection
- [x] **T15**: Contract + Policy + Resume Regression Pack (R19/S9/R20) - High *Completed (2026-02-15 implementation; TEST_SOP full gate passed 2026-02-16)*
  - **Goal**: Make high-value hardening non-regressable before enabling external enrichment by default.
  - **Plan**: `.planning/260215-S9A10R20T15_EXTERNAL_ENRICHMENT_SAFETY_FOUNDATION_PLAN.md`
  - **Implementation Record**: `.planning/260215-S9A10R20T15_EXTERNAL_ENRICHMENT_SAFETY_FOUNDATION_IMPLEMENTATION_RECORD.md`
  - **Scope**:
    - R19 contract tests: truncated output, missing markers, hidden-thinking blocks, malformed JSON, deterministic fallback assertions.
    - S9/A10 policy tests: submit/upload blocked by default, explicit-confirm token required, outbound sanitization boundary non-bypassable.
    - R20 resume tests: checkpoint persistence, restart/resume idempotency, corruption recovery, stale checkpoint cleanup.
    - Keep test harness runnable without live ComfyUI runtime (fixture + stub provider server).
  - **Acceptance**:
...
- [x] **T13**: Desktop-style Failure Injection Tests (Flush/OSError + Corrupt JSON) - ???Medium *Completed (2026-02-17; TEST_SOP gate passed)*
  ...
  - **Status (2026-02-17)**: Implemented targeted desktop failure-injection coverage (corrupt checkpoint recovery, flush-failure non-crash path, migration backup marker invariants); TEST_SOP gate passed.
...
- [x] **T9**: External Environment Test Coverage Expansion (Non-ComfyUI) - ???Medium *Completed (foundation scope, 2026-02-17; TEST_SOP gate passed)*
  - **Sprint Plan**: `.planning/260216-R17T13T9T5_NEXT_SPRINT_VALIDATION_EXPANSION_PLAN.md`
  - **Implementation Record**: `.planning/260217-R17T13T9T5_REMEDIATION_RECORD.md`
  - **Status (2026-02-17)**: Implemented foundation scope with metadata contract checks, harness payload compatibility, and pipeline isolation tests without live ComfyUI; later phase expansion remains as follow-up.
  ...
  - **Phases**:
    ...
    - **Phase 3**: SSE stub server + JS contract tests + adapter contract tests for optional providers
    - **Phase 4**: Playwright harness extensions (execution_error/polling sequences)
    - **Phase 5**: Checkpoint/resume integration tests for long-running enrichment jobs + SOP/CI updates
...
- [x] **T5**: Online API integration tests (OpenAI, DeepSeek, Anthropic) - ???Medium *Completed (opt-in lane, 2026-02-17; TEST_SOP gate passed)*
  - **Sprint Plan**: `.planning/260216-R17T13T9T5_NEXT_SPRINT_VALIDATION_EXPANSION_PLAN.md`
  - **Implementation Record**: `.planning/260217-R17T13T9T5_REMEDIATION_RECORD.md`
  - **Status (2026-02-17)**: Implemented opt-in online lane with provider-specific smoke tests (OpenAI/DeepSeek/Anthropic) + policy assertions; tests are secret-scoped and skip safely when credentials are unavailable.
  - **Extended Scope**:
    - Verify provider-specific retry/rate-limit handling (`429`, `Retry-After`, timeout envelopes).
    - Verify explicit-submit guardrails from **S9** (submit/upload routes blocked by default).
    - Run as opt-in CI track with secret-scoped credentials only.
...
- [ ] **D1**: OpenAPI/Swagger spec - ???Medium ??? *Use dev branch*
...
> [Note]
> Items marked with ??? should be developed on a separate `dev` branch. Merge to `main` only after thorough testing.
> External reference projects are design/process references only; do not copy third-party code directly into Doctor. Re-implement with ComfyUI-Doctor architecture and project coding standards.
> Items marked `Implemented (...; TEST_SOP full gate pending)` are code-complete but not acceptance-complete until `tests/TEST_SOP.md` required checks are fully recorded.
...
- [x] **STAGE 3: F12 Pattern Expansion & Full i18n** - ???Medium ??*Completed (2026-01-03)*
  ...
  - **i18n Support**:
    ...
    - ??? **UI Text**: Partial completion - See Phase 4C for remaining UI translations
...
- [x] **T2** Frontend Interaction Tests (Playwright) ??*Completed (2026-01-04; CI model updated 2026-02-10)*
  - Initial baseline: 61 end-to-end tests for Doctor UI (settings panel, chat interface, statistics dashboard, preact loader, trust/health)
  - Current dual-track model: required `npm test` (87 passed in latest baseline) + optional `npm run test:integration` (backend-dependent telemetry)
  - Integrated into CI with non-blocking integration lane
...
- [x] **F4** Statistics Dashboard ??*Completed (2026-01-04)*
  ...
  - Testing: 159/159 Python tests passed; Playwright now runs in dual-track mode (`npm test` required, `npm run test:integration` optional)
  - i18n: Fully translated across all 9 languages
  - See `.planning/260104-F4_STATISTICS_RECORD.md` for implementation details
- [x] **R6-R7** Network reliability improvements ??*Completed (2026-01-10)* (see ??.2 `R6`, `R7`)
...
- [x] **A7** Frontend Architecture Planning ??*Completed (2026-01-05)*
  ...
  - **Next Phase**: ??Completed Phase 5A??C Preact migration (2026-01-08)
...
- [x] **A7** Preact Migration (Phase 5A??C) ??*Completed (2026-01-08)*
  - **Prerequisite**: F13 (Sidebar Tab Refactoring) completed ??  - **Phase 4D**: Planning + Preact loader infrastructure ??  - **Phase 5A**: Chat/Stats islands migrated to Preact with vanilla fallback ??  - **Phase 5B**: Integration hardening (fallbacks, E2E coverage, lifecycle checks) ??  - **Phase 5C**: Extensibility layer (island registry, selectors, error boundary, E2E helpers) ??  - **Records**:
    - `.planning/260105-A7(Phase 4D)_IMPLEMENTATION_RECORD.md`
    - `.planning/260106-A7_(Phase 5A)_IMPLEMENTATION_RECORD.md`
    - `.planning/260107-A7_PHASE_5B_IMPLEMENTATION_RECORD.md`
    - `.planning/260107-A7_PHASE_5C_IMPLEMENTATION_RECORD.md`
